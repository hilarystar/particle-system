...
Particle createParticle(float screenWidth, float startY) {
    ...
    p.shape.setFillColor(sf::Color(220, 240, 255, 230)
    );
    return p;
}
...
int main() {
    ...
    while (window.isOpen()) {
        ...
        for (Particle& p : particles) {
            ...
            float dy = position.y - mouse.y;
            float distance = std::sqrt(dx * dx + dy * dy);
            const float FORCE_RADIUS = 80.0f;
            if (distance < FORCE_RADIUS && distance > 0.01f) {
                float force = (FORCE_RADIUS - distance) / FORCE_RADIUS;
                float directionX = dx / distance;
                float directionY = dy / distance;
                const float FORCE_STRENGTH = 18000.0f;
                p.velocity.x += directionX * force * FORCE_STRENGTH * dt;
                p.velocity.y += directionY * force * FORCE_STRENGTH * dt;
            }
            if (p.velocity.y < p.baseSpeed) {
                p.velocity.y += 400.0f * dt;
                    if (p.velocity.y > p.baseSpeed) {
                        p.velocity.y = p.baseSpeed;
                    }
            }
            p.velocity.x *= 0.96f;
            p.shape.move(
            ...
            if (p.shape.getPosition().y > SCREEN_H + 10.0f) {
                p = createParticle(
                    SCREEN_W,
